...
const register = async (req, res) => {
  const { username, password, name, gender, email, mobile, age } = req.body;

  // Simple validation
  if (!username || !password || !name || !gender || !email || !mobile || !age) {
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (passwordMatch) {
      ...
      const token = jwt.sign({ username: user.username, userId: user._id }, process.env.AUTH_KEY, { expiresIn: '1h' });
